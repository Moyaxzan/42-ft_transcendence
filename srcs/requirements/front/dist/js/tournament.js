export class TournamentManager {
    ...
    getCurrentMatch() {
        ...
        const unplayedMatch = this.tournamentData.matches.find(match => match.winner_id === null &&
            match.user_id !== null &&
    ...
    // Submit match result
    async submitMatchResult(matchRound, matchIndex, winnerId, scoreUser, scoreOpponent) {
        try {
    ...
    isTournamentComplete() {
        ...
        const finalMatch = this.tournamentData.matches.find(m => m.match_round === finalRound && m.match_index === 0);
    ...
    getNextMatch() {
        ...
        if (!current)
            return null;
        const currentIndex = this.tournamentData.matches.findIndex(m => m.match_round === current.match_round && m.match_index === current.match_index);
